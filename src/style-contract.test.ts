 ...
 *
 * Ported from obsidian-horizon's node:test version (wave 5), same lineage as
 * sonar/portal/masonry/tabx/horizon. Encodes only the state landed by the
 * wave-10 mv-kit audit (previous commit) — not aspirational rules the audit
 * didn't actually fix. See docs/2026-07-mv-kit-audit.md for the full
 * per-rule verdict.
 */
...
describe('mv-kit style contract', () => {
  // Regression guard (mv-kit.md's own MUST NOT, ~lines 26-34): a comment
  ...
  it('raw ms/hex/cubic-bezier values appear only as var() fallbacks', () => {
    ...
    // i.e. the line contains a var(--token, fallback) pattern. This is a
    // line-level heuristic (matches the audit procedure in mv-kit.md
    // §"Audit procedure": grep for raw values outside a var() fallback),
    // not a full CSS parse.
    ...
    lines.forEach((line, idx) => {
      ...
      // --radius-s included) — the contract's requirement is "never a bare
  ...
  it('caps !important declarations at the post-mv-kit-audit count (ratchet down only)', () => {
    ...
    // audit (2026-07): Composer's styles.css has zero !important
    // declarations, before and after this wave's fixes. Any new edit that
    // adds an !important fails this test — the ceiling can only ratchet
    // down from here (it's already at the floor).
    assert.ok(
      importantCount <= 0,
